...
std::shared_ptr<AstNode> Parser::DirectDeclarator(std::shared_ptr<CType> baseType)
{
    ...
    { // int* a[1][2]; int (*a)[1][2];
        ...
        Declarator(CType::IntType); // get type in advance like int (*a)[1][2] int (**a)[1][2];
        Consume(TokenType::r_parent);

        baseType = DirectDeclaratorSuffix(baseType); // save state to get array information
        ...
        Consume(TokenType::l_parent);
        declNode = Declarator(baseType); // now baseType is the pointer to arrayType
        Consume(TokenType::r_parent);
        DirectDeclaratorSuffix(CType::IntType); // just consume some known token
    }
    else if (tok.tokenType == TokenType::identifier)
    {
        Token iden = tok;
        Consume(TokenType::identifier);
        baseType = DirectDeclaratorSuffix(baseType); // to handle a[2][3]'s kind of arrayType
...
std::shared_ptr<AstNode> Parser::Declarator(std::shared_ptr<CType> baseType)
{
    ...
    return DirectDeclarator(baseType); // current var's type is ensured for int '(*)*'
}

std::shared_ptr<AstNode> Parser::ParseDeclStmt()
{

    auto baseTy = ParseDeclSpec(); // baseTy can only be int now

    if (tok.tokenType == TokenType::semi) // int ; for non-use
    {
...
{
    return tok.tokenType == TokenType::equal || tok.tokenType == TokenType::plus_equal || tok.tokenType == TokenType::minus_equal || tok.tokenType == TokenType::star_equal || tok.tokenType == TokenType::slash_equal || tok.tokenType == TokenType::percent_equal || tok.tokenType == TokenType::pipe_equal || tok.tokenType == TokenType::amp_equal || tok.tokenType == TokenType::caret_equal || tok.tokenType == TokenType::less_less_equal || tok.tokenType == TokenType::greater_greater_equal;
}

bool Parser::IsUnaryOperator()
{
    return tok.tokenType == TokenType::plus_plus || tok.tokenType == TokenType::minus_minus || tok.tokenType == TokenType::plus || tok.tokenType == TokenType::minus || tok.tokenType == TokenType::star || tok.tokenType == TokenType::amp || tok.tokenType == TokenType::exclaim || tok.tokenType == TokenType::tilde || tok.tokenType == TokenType::kw_sizeof;
}
